    ...
    if (drone_id_env) {
        try {
            ...
            if (drone_id >= 1 && drone_id <= 255) {
                ...
                RCLCPP_INFO(node_->get_logger(), "RTLHandler: DRONE_ID 환경 변수 사용: target_system=%d", 
                          static_cast<int>(target_system_));
            } else {
                RCLCPP_WARN(node_->get_logger(), "RTLHandler: 잘못된 DRONE_ID 값: %d (1-255 범위 필요), 기본값 1 사용", drone_id);
            }
        } catch (...) {
            RCLCPP_WARN(node_->get_logger(), "RTLHandler: DRONE_ID 환경 변수 파싱 실패, 기본값 1 사용");
        }
    } else {
...
bool RTLHandler::returnToLaunch(int timeout_ms)
{
    ...
    // GPS 위치 정보 수신 대기 (메인 executor가 자동으로 콜백 처리)
