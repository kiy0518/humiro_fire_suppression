...
안전/안정성에 영향을 주는 중요 변경이 포함된 버전은 **태그 이름** 앞에 ★를 붙여 표시:

```
태그 이름: ★v0.x.x  (예: ★v0.26.13)
```

GitHub Tags 목록에서 릴리즈 노트를 펼치지 않아도 중요 버전을 바로 식별 가능.

**★ 마크 기준**:
- 비행 안전에 영향을 주는 버그 수정 (제어 명령 교차, 착지 감지 등)
- 데이터 무결성 관련 수정 (센서 데이터 교차, 통신 오염 등)
- 아키텍처 핵심 변경 (DDS 도메인 분리, 통신 프로토콜 변경 등)
- 사용자가 "중요 버전"으로 지정한 경우

**★ 태그 생성 방법**:
```bash
git tag -a ★v0.x.x -F /tmp/tag_message.txt
git push origin ★v0.26.13
```

work-plan 문서도 중요 문서에는 제목 앞에 ★를 붙인다.
