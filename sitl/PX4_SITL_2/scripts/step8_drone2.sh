...
step8_drone2() {
    print_header
    echo -e "${BOLD}🚁 Step 8: 드론 2 실행 스크립트 생성${NC}"
    echo -e "${LINE}"
    echo ""
    echo "설정 정보:"
    echo "  - VIM4 IP: 192.168.100.21"
    echo "  - 인스턴스: -i 1"
    echo "  - MAV_SYS_ID: 2"
...
EOF
    ...
    update_status "STEP8_DRONE2"
    step_complete "드론 2 실행 스크립트 생성"
    echo -e "실행 명령: ${GREEN}~/start_sitl_drone2.sh${NC}"
}
