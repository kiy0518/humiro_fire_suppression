...
step9_drone3() {
    print_header
    echo -e "${BOLD}🚁 Step 9: 드론 3 실행 스크립트 생성${NC}"
    echo -e "${LINE}"
    echo ""
    echo "설정 정보:"
    echo "  - VIM4 IP: 192.168.100.31"
    echo "  - 인스턴스: -i 2"
    echo "  - MAV_SYS_ID: 3"
    echo -e "${YELLOW}⚠️ 드론 1이 먼저 실행된 상태에서 사용하세요!${NC}"
    echo ""
...
EOF
    ...
    update_status "STEP9_DRONE3"
    step_complete "드론 3 실행 스크립트 생성"
    echo -e "실행 명령: ${GREEN}~/start_sitl_drone3.sh${NC}"
}
