...
"""
==========================================================================
RGB + 열화상 데이터 RTSP/HTTP 스트리밍 서버 (저지연 최적화)
==========================================================================
- RGB 영상 + 열화상 데이터 오버레이
...
접속 방법:
  - RTSP: rtsp://<VIM4_IP>:8554/stream (QGC, VLC)
  - HTTP: http://<VIM4_IP>:8080 (웹 브라우저)

필요 패키지:
  sudo apt install gstreamer1.0-tools gstreamer1.0-plugins-base \
                   gstreamer1.0-plugins-good gstreamer1.0-plugins-bad \
                   gstreamer1.0-plugins-ugly libgstrtspserver-1.0-0 \
                   gir1.2-gst-rtsp-server-1.0 python3-opencv python3-flask
==========================================================================
"""
...
# OpenCV 버전에 따라 setLogLevel이 없을 수 있으므로 안전하게 처리
try:
    # OpenCV 4.5+ 버전
    if hasattr(cv2, 'setLogLevel'):
        if hasattr(cv2, 'LOG_LEVEL_ERROR'):
            cv2.setLogLevel(cv2.LOG_LEVEL_ERROR)
        elif hasattr(cv2, 'utils') and hasattr(cv2.utils, 'setLogLevel'):
            if hasattr(cv2.utils, 'LOG_LEVEL_ERROR'):
                cv2.utils.setLogLevel(cv2.utils.LOG_LEVEL_ERROR)
except:
    # OpenCV 버전이 낮거나 기능이 없으면 무시 (경고는 그대로 표시됨)
    pass

# Python warnings 억제
warnings.filterwarnings('ignore', category=UserWarning)

# FFMPEG/VideoWriter 에러 메시지 억제 (환경 변수)
import os
os.environ['OPENCV_FFMPEG_LOGLEVEL'] = '-8'  # FFMPEG 로그 레벨 최소화
os.environ['OPENCV_LOG_LEVEL'] = 'ERROR'  # OpenCV 로그 레벨
...
# 웹 브라우저용 HTTP 서버 설정
ENABLE_HTTP_SERVER = True  # True: 웹 서버 활성화, False: 웹 서버 비활성화
HTTP_PORT = 8080  # 웹 브라우저용 HTTP 포트
...
flask_app = None  # Flask 웹 서버
flask_thread = None  # Flask 스레드
web_frame_queue = None  # 웹 서버용 별도 프레임 큐 (지연 최소화)
...
thermal_data = {
    ...
    'frame': None       # 열화상 프레임 (오버레이용)
}
thermal_data_lock = threading.Lock()  # 열화상 데이터 동기화용

# 열화상 오버레이 설정
OVERLAY_THERMAL = True  # True: 열화상 영상 오버레이, False: 데이터만 표시
ALPHA_THERMAL_LAYER = 0.5  # 열화상 오버레이 투명도 (0.0~1.0)
# 0.0 = 완전 투명 (RGB만 보임), 1.0 = 완전 불투명 (열화상만 보임)
# 0.3 = 열화상 약하게, 0.5 = 중간, 0.7 = 열화상 강하게

GRADIENT_BORDER_SIZE = 30  # 그라데이션 테두리 크기 (픽셀)
# 작은 값 (10-20): 날카로운 경계, 짧은 그라데이션
# 중간 값 (30-50): 부드러운 경계, 적당한 그라데이션
# 큰 값 (60-100): 매우 부드러운 경계, 긴 그라데이션

GRADIENT_CURVE = 1.0  # 그라데이션 곡선 (1.0=선형, 2.0=제곱, 0.5=제곱근)
# 1.0 = 선형 그라데이션 (일정한 속도로 페이드)
# 2.0 = 제곱 곡선 (빠르게 시작, 천천히 끝남)
# 0.5 = 제곱근 곡선 (천천히 시작, 빠르게 끝남)
...
def find_sd_card_path():
    ...
    # 일반적인 SD 카드 마운트 경로
    possible_paths = [
        "/media",  # Ubuntu/Debian 일반 경로
        "/mnt",    # 일반 마운트 경로
        "/run/media",  # systemd 마운트 경로
    ]
    ...
    try:
        ...
        if result.returncode == 0:
            for line in result.stdout.split('\n'):
                # SD 카드나 외부 저장 장치 찾기 (일반적으로 /dev/mmcblk 또는 /dev/sd로 시작)
                if '/dev/mmcblk' in line or ('/dev/sd' in line and 'media' in line.lower()):
                    parts = line.split()
                    if len(parts) >= 6:
                        mount_point = parts[5]
                        # 쓰기 가능한지 확인
    ...
    # 가능한 경로에서 쓰기 가능한 디렉토리 찾기
    for base_path in possible_paths:
        if os.path.exists(base_path) and os.access(base_path, os.W_OK):
            # 하위 디렉토리 확인
            try:
    ...
    # 찾지 못하면 홈 디렉토리의 videos 폴더 사용
    home_videos = os.path.join(os.path.expanduser("~"), "videos")
    try:
        os.makedirs(home_videos, exist_ok=True)
        if os.access(home_videos, os.W_OK):
            return home_videos
    except:
        pass
...
def init_cameras():
    ...
    try:
        ...
        # Lepton 3.5는 느리므로 FPS 설정 (8-9 FPS)
        cap_thermal.set(cv2.CAP_PROP_FPS, 9)
    except:
        pass
    ...
    print("      → Lepton 3.5는 8-9 FPS로 느려서 타임아웃 경고가 나타날 수 있습니다 (정상 동작)")
    # 열화상은 느리므로 더 긴 대기 시간
    for i in range(5):
        # 타임아웃 경고 억제 (OpenCV 버전에 따라 다름)
        ret, _ = cap_thermal.read()
        
        if ret:
            print(f"      → 프레임 {i+1}/5 읽기 성공")
        else:
            print(f"      → 프레임 {i+1}/5 읽기 실패 (재시도 중...) - 정상 (카메라가 느림)")
        time.sleep(0.15)  # Lepton 3.5는 약 0.12초 간격 (8.3 FPS)
    ...
    for attempt in range(3):
        ...
        if ret1 and f1 is not None:
            break
        print(f"      → 열화상 읽기 시도 {attempt+1}/3... (타임아웃 경고는 정상)")
        time.sleep(0.3)
...
def extract_thermal_data(frame_thermal):
    """열화상 프레임에서 데이터 추출 (화점, 온도 등) 및 프레임 저장"""
    ...
    try:
        ...
        # 열화상 프레임 준비 (오버레이용)
        thermal_frame_for_overlay = None
        if OVERLAY_THERMAL:
            # 원본 열화상 프레임을 오버레이 크기로 리사이즈
            # RGB 영상 크기: 640x480
            # 오버레이 위치: (RGB_CROP_X, RGB_CROP_Y)
            # 오버레이 크기: THERMAL_WIDTH x THERMAL_CROPPED_HEIGHT
            thermal_frame_for_overlay = cv2.resize(frame_thermal, 
                                                    (THERMAL_WIDTH, THERMAL_CROPPED_HEIGHT),
        ...
        with thermal_data_lock:
            ...
            thermal_data['frame'] = thermal_frame_for_overlay  # 오버레이용 프레임 저장
...
def composite_frames_optimized(frame_rgb):
    """RGB 영상 + 저장된 열화상 데이터 표시 (이전 값 유지, 오버레이 옵션)"""
    global thermal_data, thermal_data_lock
    ...
    with thermal_data_lock:
        ...
        thermal_frame = data.get('frame')  # 열화상 프레임 (오버레이용)
    
    # 열화상 영상 오버레이 (옵션)
    if OVERLAY_THERMAL and thermal_frame is not None and data['valid']:
        try:
            # 열화상 프레임을 RGB 좌표계에 맞게 배치
            overlay_x = RGB_CROP_X
            overlay_y = RGB_CROP_Y
            
            # 저장된 열화상 프레임의 크기 확인
            thermal_h, thermal_w = thermal_frame.shape[:2]
            
            # 오버레이할 크기 (경계 체크)
            overlay_w = min(THERMAL_WIDTH, 640 - overlay_x, thermal_w)
            overlay_h = min(THERMAL_CROPPED_HEIGHT, 480 - overlay_y, thermal_h)
            
            if overlay_w > 0 and overlay_h > 0:
                # 열화상 프레임이 저장된 크기와 다르면 리사이즈
                if thermal_w != overlay_w or thermal_h != overlay_h:
                    thermal_resized = cv2.resize(thermal_frame, (overlay_w, overlay_h), 
                                                interpolation=cv2.INTER_LINEAR)
                else:
                    thermal_resized = thermal_frame
                
                # ROI 범위 확인
                if (overlay_y + overlay_h <= output.shape[0] and 
                    overlay_x + overlay_w <= output.shape[1]):
                    # ROI 추출
                    roi = output[overlay_y:overlay_y+overlay_h, overlay_x:overlay_x+overlay_w]
                    
                    # 알파 블렌딩
                    if len(thermal_resized.shape) == 3 and thermal_resized.shape[2] == 3:
                        # BGR 형식인 경우
                        if roi.shape == thermal_resized.shape:
                            # 그라데이션 마스크 생성 (모서리 페이드 아웃)
                            mask = np.ones((overlay_h, overlay_w), dtype=np.float32)
                            
                            # 그라데이션 크기 (경계에서 페이드)
                            grad_size = min(GRADIENT_BORDER_SIZE, overlay_w // 4, overlay_h // 4)
                            
                            if grad_size > 0:
                                # 그라데이션 곡선 함수
                                def apply_curve(value):
                                    """그라데이션 곡선 적용"""
                                    if GRADIENT_CURVE == 1.0:
                                        return value  # 선형
                                    elif GRADIENT_CURVE > 1.0:
                                        return np.power(value, GRADIENT_CURVE)  # 제곱
                                    else:
                                        return np.power(value, GRADIENT_CURVE)  # 제곱근
                                
                                # 상단 그라데이션
                                for y in range(grad_size):
                                    alpha = apply_curve(y / grad_size)
                                    mask[y, :] *= alpha
                                
                                # 하단 그라데이션
                                for y in range(overlay_h - grad_size, overlay_h):
                                    alpha = apply_curve((overlay_h - y) / grad_size)
                                    mask[y, :] *= alpha
                                
                                # 좌측 그라데이션
                                for x in range(grad_size):
                                    alpha = apply_curve(x / grad_size)
                                    mask[:, x] *= alpha
                                
                                # 우측 그라데이션
                                for x in range(overlay_w - grad_size, overlay_w):
                                    alpha = apply_curve((overlay_w - x) / grad_size)
                                    mask[:, x] *= alpha
                            ...
                            # 그라데이션 적용된 알파 블렌딩
                            alpha_mask = mask_3d * ALPHA_THERMAL_LAYER
                            blended = (roi * (1.0 - alpha_mask) + 
                                     thermal_resized * alpha_mask).astype(np.uint8)
                            
                            output[overlay_y:overlay_y+overlay_h, overlay_x:overlay_x+overlay_w] = blended
                        else:
                            print(f"  ⚠ 크기 불일치: ROI {roi.shape} vs Thermal {thermal_resized.shape}")
        except Exception as e:
            print(f"  ⚠ 열화상 오버레이 오류: {e}")
            import traceback
            traceback.print_exc()
...
def thermal_capture_thread():
    """열화상 카메라 캡처 스레드 (느린 응답 대비, 타임아웃 경고 억제)"""
    global cap_thermal, is_running
    
    # 타임아웃 경고는 OpenCV 버전에 따라 억제됨 (Lepton 3.5는 느려서 정상)
    ...
    while is_running:
        try:
            ...
            if not ret or frame_thermal is None or frame_thermal.size == 0:
                ...
                if consecutive_failures >= max_failures:
                    ...
                    try:
                        ...
                        print(f"  ⚠ 열화상 재연결 실패: {e}")
                time.sleep(0.2)  # 실패 시 더 긴 대기
                continue
            ...
            # Lepton 3.5는 8-9 FPS이므로 적절한 대기 (약 0.12초 = 8.3 FPS)
            time.sleep(0.12)
...
def composite_thread():
    ...
    while is_running:
        try:
            ...
            # 웹 서버용 큐에 넣기 (활성화된 경우만, 최신 프레임만 유지)
            if ENABLE_HTTP_SERVER and web_frame_queue is not None:
                if web_frame_queue.full():
                    try:
                        web_frame_queue.get_nowait()
                    except:
                        pass
                try:
                    web_frame_queue.put_nowait(composite)
                except:
                    pass  # 큐가 가득 차면 스킵 (최신 프레임만 유지)
...
# ============================================================
# 웹 브라우저용 HTTP 스트리밍 (Flask)
# ============================================================
def create_flask_app():
    """Flask 웹 서버 생성 (MJPEG 스트리밍)"""
    app = Flask(__name__)
    ...
    <body>
        <div class="container">
            ...
            <div class="info">
                <p><strong>RTSP URL:</strong> rtsp://{{ rtsp_ip }}:{{ rtsp_port }}{{ rtsp_path }}</p>
                <p><strong>HTTP 스트림:</strong> http://{{ http_ip }}:{{ http_port }}/video_feed</p>
            </div>
        </div>
    </body>
    </html>
    ...
    def video_feed():
        """MJPEG 스트림 엔드포인트"""
        def generate():
            global web_frame_queue, is_running
            while is_running:
                try:
                    # 프레임 가져오기 (타임아웃 1초) - 별도 큐 사용
                    if web_frame_queue is None:
                        time.sleep(0.1)
                        continue
                    frame = web_frame_queue.get(timeout=1.0)
                    
                    if frame is None or frame.size == 0:
                        continue
                    
                    # frame_queue에는 RGB 형식으로 저장되어 있음
                    # cv2.imencode는 BGR을 기대하지만, JPEG는 색상 순서를 보존하므로
                    # RGB를 그대로 인코딩하면 웹에서 RGB로 표시될 때 색상이 반대로 됨
                    # 따라서 RGB → BGR 변환 후 인코딩하면 웹에서 정상 색상으로 표시됨
                    if len(frame.shape) == 3:
                        if frame.shape[2] == 3:
                            # RGB → BGR 변환 (cv2.imencode는 BGR 기대)
...
def start_flask_server():
    """Flask 웹 서버 시작 (별도 스레드)"""
    global flask_app, flask_thread, is_running, web_frame_queue
    
    try:
        # 웹 서버용 별도 프레임 큐 생성 (RTSP와 경쟁 방지)
        web_frame_queue = Queue(maxsize=1)  # 최신 프레임만 유지
        
        flask_app = create_flask_app()
        # Flask를 별도 스레드에서 실행 (메인 스레드가 아닌 곳에서)
        ...
        flask_thread.start()
        print(f"  ✓ HTTP 웹 서버 시작: http://{get_local_ip()}:{HTTP_PORT}")
        print(f"     → 웹 서버는 별도 큐 사용 (RTSP 지연 영향 최소화)")
        return True
    except Exception as e:
        print(f"  ⚠ HTTP 웹 서버 시작 실패: {e}")
        return False
...
def main():
    ...
    # 웹 브라우저용 HTTP 서버 시작 (설정에 따라)
    http_url = None
    if ENABLE_HTTP_SERVER:
        print("\n[HTTP 웹 서버]")
        if start_flask_server():
            http_url = f"http://{local_ip}:{HTTP_PORT}"
            time.sleep(1)  # Flask 서버 시작 대기
        else:
            print("  ⚠ HTTP 웹 서버 시작 실패 (계속 진행)")
    
    print("\n" + "=" * 60)
    print("  ✓ RTSP 서버 시작됨")
    if ENABLE_HTTP_SERVER and http_url:
        print("  ✓ HTTP 웹 서버 시작됨")
    print("=" * 60)
    print(f"\n  ★ RTSP URL: {rtsp_url}")
    if http_url:
