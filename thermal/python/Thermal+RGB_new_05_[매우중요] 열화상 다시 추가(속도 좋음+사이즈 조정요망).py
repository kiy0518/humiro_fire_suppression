...
def composite_frames_optimized(frame_rgb):
    ...
    if OVERLAY_THERMAL and thermal_frame is not None and data['valid']:
        try:
            ...
            # 경계 체크
            overlay_w = min(THERMAL_WIDTH, 640 - overlay_x)
            overlay_h = min(THERMAL_CROPPED_HEIGHT, 480 - overlay_y)
            ...
            if overlay_w > 0 and overlay_h > 0:
                ...
                if len(thermal_resized.shape) == 3 and thermal_resized.shape[2] == 3:
                    # BGR 형식인 경우
                    blended = cv2.addWeighted(roi, 1.0 - ALPHA_THERMAL_LAYER, 
                                           thermal_resized, ALPHA_THERMAL_LAYER, 0)
