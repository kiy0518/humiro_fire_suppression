...
def composite_frames_optimized(frame_thermal, frame_rgb):
    """RGB 영상 + 열화상 데이터만 표시 (오버레이 없음, 데이터만)"""
    
    # 프레임 검증
    if frame_rgb is None or len(frame_rgb.shape) < 2 or frame_rgb.size == 0:
        print("  ⚠ RGB 프레임 오류 - RGB 영상만 반환")
        # RGB 프레임이 없으면 None 반환 (상위에서 처리)
        return None
    ...
    cv2.putText(output, f"Offset: ({rel_x:3d}, {rel_y:3d})", (10, text_y), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    # 온도값 정보
    text_y += text_spacing
    cv2.rectangle(output, (5, text_y-15), (200, text_y+10), (0, 0, 0), -1)
    cv2.putText(output, f"Max: {max_val:.1f}", (10, text_y), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    # 최소값 정보
    text_y += text_spacing
    cv2.rectangle(output, (5, text_y-15), (200, text_y+10), (0, 0, 0), -1)
    cv2.putText(output, f"Min: {min_val:.1f}", (10, text_y), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
def camera_thread():
    ...
    while is_running:
        # RGB 프레임 먼저 읽기 (RGB가 메인 영상)
        ret2, frame_rgb = cap_rgb.read()
        
        if not ret2 or frame_rgb is None or frame_rgb.size == 0:
        ...
        if rgb_h < 200 or rgb_w < 200:
            print(f"  ⚠ RGB 프레임이 너무 작습니다: {frame_rgb.shape} - 스킵")
            time.sleep(0.1)
            continue
        ...
        if not ret1 or frame_thermal is None or frame_thermal.size == 0:
            # 열화상이 없어도 RGB만 전송 (타겟팅 마커 없이)
            try:
                output = frame_rgb.copy()
                if output.shape[:2] != (OUTPUT_HEIGHT, OUTPUT_WIDTH):
                    output = cv2.resize(output, (OUTPUT_WIDTH, OUTPUT_HEIGHT), interpolation=cv2.INTER_LINEAR)
        ...
        try:
            ...
            if frame_thermal is None or frame_thermal.size == 0:
                print("  ⚠ 열화상 프레임이 비어있음")
                time.sleep(0.1)
                continue
            ...
            if now - last_time >= 5.0:
                ...
                print(f"     RGB: {frame_rgb.shape}, Thermal: {frame_thermal.shape}, Output: {composite.shape}")
                frame_count = 0
                last_time = now
