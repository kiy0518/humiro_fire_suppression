...
# device_config.env에서 FC_IP 읽기 (환경 변수 사용)
...
if ! ip addr show eth0 | grep -q "10.0.0"; then
    ...
    echo "  해결: sudo $SCRIPTS_INSTALL/003-apply_config.sh"
    ISSUES=$((ISSUES + 1))
fi
...
if [ -z "$AGENT_PID" ]; then
    ...
    echo "  해결: $SCRIPTS_RUNTIME/start_micro_ros_agent_wrapper.sh"
    ISSUES=$((ISSUES + 1))
fi
...
if [ "$TOPICS_NUM" -eq 0 ] 2>/dev/null; then
    echo "✗ PX4 토픽 없음"
    echo ""
    # device_config.env에서 ETH0_IP 읽기 (환경 변수 사용)
    CONFIG_FILE="$DEVICE_CONFIG"
    ETH0_IP=""
    if [ -f "$CONFIG_FILE" ]; then
