...
# 메인앱 전용 FastDDS 프로파일 (WiFi만 - Domain 1 편대 통신)
# DDS Domain 분리: 메인앱은 Domain 1 (WiFi), fc_bridge는 Domain 0 (loopback)
FASTDDS_WIFI_XML="$PROJECT_ROOT/config/fastdds_wifi_only.xml"

cat > "$FASTDDS_WIFI_XML" << EOF
<?xml version="1.0" encoding="UTF-8" ?>
<!--
  메인 애플리케이션 전용 FastDDS 설정 (Domain 1)
  편대 토픽을 WiFi로만 전송. FC 통신은 fc_bridge (Domain 0, loopback)에서 처리.
  Generated by 003-apply_config.sh
-->
<dds>
    <profiles xmlns="http://www.eprosima.com/XMLSchemas/fastRTPS_Profiles">
        <transport_descriptors>
            <transport_descriptor>
                <transport_id>wifi_only_udp</transport_id>
                <type>UDPv4</type>
                <interfaceWhiteList>
                    <address>$WIFI_IP</address>
                </interfaceWhiteList>
...
chown $REAL_USER:$REAL_USER "$FASTDDS_WIFI_XML"
echo "  ✓ 메인앱 FastDDS 프로파일 생성 (WiFi 전용: $WIFI_IP, Domain 1)"
...
# .bashrc에서 전역 설정하면 ros2-daemon 등이 WiFi DDS에 참여하여 교차오염 발생
...
# fc-bridge.service 업데이트 (DDS Domain 분리)
if [ -f "$PROJECT_DEPLOYMENT/systemd/fc-bridge.service" ]; then
    sed "s|%h|$REAL_HOME|g; s|khadas|$REAL_USER|g" "$PROJECT_DEPLOYMENT/systemd/fc-bridge.service" | \
        sudo tee /etc/systemd/system/fc-bridge.service > /dev/null
    echo "  ✓ fc-bridge.service 업데이트 완료"
fi
