...
└────┬────┘
     │ 명령/상태 공유
┌────┴────┬─────────┐
│ 팔로워L │ 팔로워R │ ← 리더 상태 확인 후 따라가기
└─────────┴─────────┘
```
- 기본 편대 비행 (리더 WAYPOINT 기준 오프셋 적용)
- 상태 동기화 (ARM → TAKEOFF → 이동 → 조준 → 발사)
- 동기화된 발사 (리더 명령에 의한 동시 발사)
- Fail-safe (리더 통신 끊김 시 hover → RTL)

### Level 2: 분산 협조 (다음 단계)
```
┌─────────┐ ┌─────────┐ ┌─────────┐
│  기체1  │↔│  기체2  │↔│  기체3  │ ← 상호 정보 공유
└─────────┘ └─────────┘ └─────────┘
     │           │           │
  개별 판단    개별 판단    개별 판단
```
...
```
┌─────────┐ ┌─────────┐ ┌─────────┐
│  기체1  │ │  기체2  │ │  기체3  │ ← 독립적 AI 의사결정
└─────────┘ └─────────┘ └─────────┘
     │           │           │
  AI 판단     AI 판단     AI 판단
```
- AI 기반 타겟 자동 인식 (열화상/비전)
- 자율 위치 선정 및 최적 경로 계산
- 상황 적응형 임무 수행 (화재 확산, 바람 등 대응)
- 분산 합의 알고리즘
