...
커스텀 메시지 라이브러리는 **표준 MAVLink 메시지는 필터링하지 않고** MAVLink 라우터가 자동으로 처리하도록 설계되었습니다. 이 프로그램은 **커스텀 메시지(12900-12903)와 COMMAND_LONG(76)만 처리**합니다.

### 핵심 원칙

1. **표준 메시지**: 이 프로그램에서 필터링하지 않고 통과시킴 → MAVLink 라우터가 자동으로 FC↔QGC 간 전달
2. **커스텀 메시지**: 이 프로그램에서 파싱 및 콜백 호출
3. **COMMAND_LONG**: 이 프로그램에서 파싱 및 콜백 호출 (ARM/DISARM 등)
...
수신된 MAVLink 메시지
    ...
    ├─ MSG_ID = 0 → 무시 (잘못된 메시지)
    │
    ├─ MSG_ID = 76 (COMMAND_LONG) → 파싱 + 콜백 호출
    │
    ├─ MSG_ID = 12900-12903 (커스텀) → 파싱 + 콜백 호출
    │
    └─ 그 외 표준 메시지 → 필터링하지 않고 통과 (MAVLink 라우터가 처리)
```
...
└────────┬────────┘
    ...
    │          VIM4 (192.168.100.11)          │
    │  ┌────────────────────────────────────┐ │
    │  │    MAVLink Router (systemd)        │ │
    │  │  • FC: 14540 (Server)              │ │
    │  │  • QGC: 192.168.100.255:14550      │ │
    │  │  • ROS2: 127.0.0.1:14551           │ │
    │  └────────────────────────────────────┘ │
    │           │                              │
    │           │ 표준 메시지 자동 전달         │
    │           │                              │
    │  ┌────────▼──────────────────────────┐ │
    │  │  Custom Message Library (14550)   │ │
    │  │  • 커스텀 메시지: 파싱 + 콜백     │ │
    │  │  • COMMAND_LONG: 파싱 + 콜백      │ │
    │  │  • 표준 메시지: 통과 (필터링 안함)│ │
    │  └────────────────────────────────────┘ │
...
```
QGC ──────[WiFi 14550]──────> MAVLink Router ──────[Ethernet 14540]──────> FC
                                        │
                                        │ 동시에 브로드캐스트
                                        ▼
                                Custom Message Library
                                (필터링하지 않고 통과)

FC ──────[Ethernet 14540]──────> MAVLink Router ──────[WiFi 14550]──────> QGC
                                        │
                                        │ 동시에 브로드캐스트
                                        ▼
                                Custom Message Library
...
**특징**:
- MAVLink 라우터가 표준 메시지를 **자동으로 FC↔QGC 간 전달**
- Custom Message Library는 표준 메시지를 **필터링하지 않고 통과**
- VIM4 프로그램이 실행되지 않아도 표준 메시지는 정상 작동

#### 커스텀 메시지 경로 (Custom Message Library 처리)

```
QGC ──────[WiFi 14550]──────> MAVLink Router ──────[브로드캐스트]──────> Custom Message Library
                                                                              │
                                                                              │ 파싱 + 콜백
                                                                              ▼
                                                                      Application Code
...
Application Code ──[송신]──> Custom Message Library ──────[WiFi 14550]──────> MAVLink Router ──────> QGC
```

**특징**:
- 커스텀 메시지(12900-12903)는 Custom Message Library에서 **파싱 및 콜백 호출**
- COMMAND_LONG(76)도 **파싱 및 콜백 호출**
- 표준 메시지와 독립적으로 동작
...
| 포트 | 용도 | 프로그램 | 설명 |
|------|------|----------|------|
| **14540** | FC ↔ VIM4 | MAVLink Router | Ethernet, Server 모드 (FC 브로드캐스트 수신) |
| **14550** | VIM4 ↔ QGC | MAVLink Router<br>Custom Message Library | WiFi, 브로드캐스트 (192.168.100.255) |
| **14551** | ROS2 | MAVLink Router | 로컬호스트 (127.0.0.1) |
| **5790** | TCP | MAVLink Router | TCP 서버 (선택적) |

### VIM4 프로그램 독립성

#### 표준 메시지 (VIM4 프로그램 불필요)

```
✅ VIM4 프로그램 실행 안 함
   └─> 표준 메시지는 MAVLink Router가 자동으로 FC↔QGC 간 전달
   └─> Custom Message Library는 표준 메시지를 필터링하지 않음
   └─> QGC에서 드론 기본 제어 (ARM, TAKEOFF, LAND 등) 정상 작동
```

#### 커스텀 메시지 (VIM4 프로그램 필요)

```
⚠️ VIM4 프로그램 실행 안 함
   └─> 커스텀 메시지는 Custom Message Library에서 수신
   └─> 콜백 함수가 등록되지 않아서 처리되지 않음
   └─> 화재 진압 미션 기능은 작동하지 않음
```
...
void parseMAVLinkMessage(const uint8_t* buffer, size_t len) {
    ...
    uint32_t msg_id = header->msgid | (header->msgid_ext[0] << 8) | 
                     (header->msgid_ext[1] << 16);
    ...
    // 표준 MAVLink 메시지는 필터링하지 않고 그대로 통과
    // MAVLink 라우터가 자동으로 FC↔QGC 간 표준 메시지를 전달
    bool is_custom_message = (msg_id >= 12900 && msg_id <= 12903);
    bool is_command_long = (msg_id == 76);
    
    // 표준 메시지는 파싱하지 않고 그대로 통과 (MAVLink 라우터가 처리)
    if (!is_custom_message && !is_command_long) {
        return;  // 필터링하지 않고 통과
    }
    
    // 커스텀 메시지와 COMMAND_LONG만 파싱 및 콜백 호출
    // ... 파싱 로직 ...
}
```
...
1. **독립성**: VIM4 프로그램이 실행되지 않아도 표준 메시지는 정상 작동
2. **효율성**: 표준 메시지는 불필요하게 파싱하지 않음
3. **안정성**: MAVLink 라우터가 검증된 메시지 전달 보장
4. **명확한 역할 분리**: 커스텀 메시지만 이 프로그램에서 처리

### 요약

| 메시지 타입 | MSG_ID 범위 | 필터링 여부 | 처리 방식 |
|------------|------------|------------|----------|
| **표준 메시지** | 1-75, 77-12899, 12904+ | ❌ 필터링 안 함 | MAVLink 라우터가 자동 전달 |
| **COMMAND_LONG** | 76 | ✅ 파싱 | 파싱 + 콜백 호출 |
| **커스텀 메시지** | 12900-12903 | ✅ 파싱 | 파싱 + 콜백 호출 |
| **잘못된 메시지** | 0 | ❌ 무시 | 통계만 업데이트 |
